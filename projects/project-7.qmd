...
Framework/Stack: Next.js (App Router) + TypeScript, Tailwind CSS

Backend: Supabase (Postgres, Row Level Security), Server Actions / API routes

Auth: Clerk (users, KYC/ID-flag ready)

Payments: Stripe (holds, charges, adjustments, refunds)

Storage: Supabase Storage (tool photos, pickup/return condition photos)

Lock Access: Vendor API for time-bound PIN codes (avoid BLE for MVP)

Deployment: Vercel

Tools Used:
- Claude Code and ChatGPT for development assistance
